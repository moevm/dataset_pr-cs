...
void del_even(char** text, int index){ 
    for(int i=0;i<index;i++){
        int counter = w_counter(text[i]);
...
void sort_by_cap_let(char** text, int index) {
	for (int i = 0; i < index; i++){
		int num_words = w_counter(text[i]);
...
void less_then_3(char** text, int index){
    for(int i=0;i<index;i++){
        int str_capacity = strlen(text[i]) + w_counter(text[i])*11;
        int str_last_index = 0;
        int counter = 0;
        char* cur_line = malloc((strlen(text[i]) + w_counter(text[i])*11) * sizeof(char));
...
void substrings_numbers(char** text, int index){
...
int counter_uppers(char* sentence) {
	int c_up = 0;
...
void nine(char** text, int index, int uppers) {
	for(int i = 0; i < index; i++) {
		if (counter_uppers(text[i]) <= uppers) {
			puts(text[i]);
		}
		free(text[i]);
	}
...
int main() {
    printf("Course work for option 4.23, created by Daniil Saldin.\n");
	...
	case 9:{
		...
		char** text = get_text(&index);
		nine(text, index, uppers);
		break;
	}
	default:{
		printf("%s", ERR_COMMAND_DOES_NOT_EXIST);
	}
    }
    return 0;
}
