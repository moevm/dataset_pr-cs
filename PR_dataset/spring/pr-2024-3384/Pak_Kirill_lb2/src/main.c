...
void removeEl(MusicalComposition* head, char* name_for_remove){
    MusicalComposition* cur = head;
    while((cur!= NULL) && (strcmp(cur->name, name_for_remove) != 0))cur = cur->next;
    if (cur == NULL) return;
    else{
        MusicalComposition* new_prev=cur->prev;
        MusicalComposition* new_next=cur->next;
        free(cur);
        if(new_prev) new_prev->next=new_next;
        if(new_next) new_next->prev=new_prev;
    }
}
