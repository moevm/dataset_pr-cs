...
void printhelp(){
    puts(
        ...
        //"--circle_pixel       Surrounds all pixels with a given color with a frame of a different color\n"
        //"   --size                Stroke thickness\n"
        //"   --color               Desired color (format: \"rrr.ggg.bbb\")\n"
        //"   --circle_color        Stroke color (format: \"rrr.ggg.bbb\")"
    );
}
...
char processopts(int argc, char **argv, Argf *argf){
    ...
    // TODO: Обратить внимание на: color дублируется в функ на 4
    int c;
    ...
    char has_gamma = 0;
    char has_value = 0;
    //char has_circle_pixel = 0;
    //char has_only_circle_pixel_opts = 0;
    //char has_size = 0;
    //char has_circle_color = 0;
    ...
    while (1)
    {
        ...
        case 'e':
        {
            ...
            if (sscanf(optarg, "%f", &argf->value) != 1){
                fprintf(stderr, "Invalid argument (value is not float). Error 45\n");
                exit(45);
            }
            if (argf->value < 0){
                fprintf(stderr, "Invalid argument (no value). Error 45\n");
                exit(45);
            }
            has_value = 1;
            break;
    ...
    if (has_help && (has_only_squared_lines_opts || has_only_rgbfilter_opts || has_only_rotate_opts || has_input || has_output || has_info || has_left_up || has_gamma || has_value)){
        fprintf(stderr, "Invalid flag. Error 46\n");
        exit(46);
    }

    if (has_info && (has_only_squared_lines_opts || has_only_rgbfilter_opts || has_only_rotate_opts || has_output || has_left_up || has_gamma || has_value)){
        fprintf(stderr, "Invalid flag. Error 46\n");
        exit(46);
    }
    ...
    if ((has_gamma || has_value) && (has_only_squared_lines_opts || has_left_up || has_only_rgbfilter_opts || has_only_rotate_opts)){
        fprintf(stderr, "Invalid flag. Error 46\n");
        exit(46); 
    }
