...
void gamma_(Rgb ***bmp_file_pixels, BitmapInfoHeader bmif, float value){
    int new_r;
    int new_g;
    int new_b;
    for (unsigned int y = 0; y<bmif.height; y++){
        for (unsigned int x = 0; x<bmif.width; x++){
            (*bmp_file_pixels)[y][x].r = (int)(powf((*bmp_file_pixels)[y][x].r / (float)255, value) * 255);
            (*bmp_file_pixels)[y][x].g = (int)(powf((*bmp_file_pixels)[y][x].g / (float)255, value) * 255);
            (*bmp_file_pixels)[y][x].b = (int)(powf((*bmp_file_pixels)[y][x].b / (float)255, value) * 255);
        }
    }
}
...
int main(int argc, char **argv){
    ...
    else if (argf.function == 'g'){
        gamma_(&bmp_file_pixels, bmif, argf.value);
    }
