...
void removeEl(MusicalComposition* head, char* name_for_remove){
    while(head->next != NULL && strcmp(head->name, name_for_remove) != 0){
        head = head->next;
    }
    if (strcmp(head->name, name_for_remove) != 0) {
        return;
    }
    head->next->previous = head->previous == NULL ?  NULL : head->previous;
    head->previous->next = head->next == NULL ? NULL : head->next;
    free(head);
};
