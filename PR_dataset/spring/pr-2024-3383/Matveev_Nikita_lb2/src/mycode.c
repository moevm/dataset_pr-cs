...
MusicalComposition* createMusicalComposition(char* name, char* author,int year){
    ...
    new_track->name = (char*)malloc(strlen(name)*sizeof(char)+1);
    new_track->author = (char*)malloc(strlen(author)*sizeof(char)+1);
    strcpy(new_track->name, name);
    strcpy(new_track->author, author);
    new_track->year = year;
    new_track->next = NULL;
    new_track->prev = NULL;
    return new_track;
...
void removeEl(MusicalComposition* head, char* name_for_remove){
    ...
    while(strcmp(p->name, name_for_remove)!=0){
        p = p->next;
    }


    if (p!= NULL){p->prev->next = p->next; free(p);}
