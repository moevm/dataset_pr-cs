...
int get_path(char *curr_dir, char *filename, char *path) {
    DIR *dir;
    ...
    while ((curr_file = readdir(dir)) != NULL) {
        if (curr_file->d_type == DT_DIR) {
            ...
            snprintf(next_dir, sizeof(next_dir), "%s/%s", curr_dir, curr_file->d_name);
            if (get_path(next_dir, filename, path)) {
                closedir(dir);
                return 1;
            }
        } else {
            if (strcmp(curr_file->d_name, filename) == 0) {
                char file_path[MAX_LEN];
                snprintf(file_path, sizeof(file_path), "%s/%s", curr_dir, curr_file->d_name);
...
int find_minotaur(char *root_dir, char *filename) {
    char file_path[MAX_LEN];
    if (get_path(root_dir, filename, file_path) == 0)
        return 0;
...
int main() {
    ...
    if (find_minotaur("./labyrynth", "file.txt")) {
        for (int i = 0; i < ind; i++) {
            fputs(result[i], file);
            fputs("\n", file);
        }
