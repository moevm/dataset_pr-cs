...
char *fin[MAX_LEN];
int ind = 0;
int PR_path(char *curr_dir, char *filename, char *path) 
{
    DIR *dir;
    struct dirent *entry;
    if ((dir = opendir(curr_dir)) == NULL){return 0;}
    while ((entry = readdir(dir)) != NULL) {
        if (entry->d_type == DT_DIR) 
        {
            ...
            if (PR_path(next_dir, filename, path)) 
            {
                closedir(dir);
                return 1;
            }
...
int find_Minotaur(char *root_dir, char *filename) {
    char file_path[MAX_LEN];
    if (PR_path(root_dir, filename, file_path) == 0){return 0;}
    FILE *file = fopen(file_path, "r");
    if (file == NULL){return 0;}
    int fl = 1;
    for (int i = 0; i < ind; i++) 
    {
        if (strcmp(fin[i], file_path) == 0) 
        {
            fl = 0;
            break;
        }
