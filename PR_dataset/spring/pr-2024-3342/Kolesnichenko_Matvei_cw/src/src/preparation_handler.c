...
void handle_arguments(int argc, char *argv[], Options *options) {
    ...
    const struct option long_options[] = {
        ...
        {"outside_ornament", no_argument, NULL, 271},
        {NULL, 0, NULL, 0}
    };
    ...
    while ((res = getopt_long(argc, argv, short_options, long_options, NULL)) != -1) {
        switch (res) {
            ...
            case 270: /* --info */
                options->flag_info = 1;
                break;
            case 271: /* --outside_ornament */
                if (options->flag_filled_rects ||options->flag_info || options->flag_copy || options->flag_ornament || options->flag_color_replace) {
                    printf("Error: Cannot use more than one function simultaneously\n");
                    exit(ERR_INSUFFICIENT_ARGUMENTS);
                }
    ...
    if (!options->flag_outside_ornament && !options->flag_info && !options->flag_help && !options->flag_copy && !options->flag_color_replace && !options->flag_ornament && !options->flag_filled_rects) {
    ...
    if (options->flag_outside_ornament && (!options->flag_thickness || !options->flag_color)) {
        printf("Error: Insufficient arguments for --outside_ornament\n");
        exit(ERR_INSUFFICIENT_ARGUMENTS);
    }
