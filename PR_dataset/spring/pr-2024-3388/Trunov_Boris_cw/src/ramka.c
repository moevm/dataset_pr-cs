...
void draw_ramka(image_bmp * image, int thick, rgb color, char * output_file_name){
		int up = thick;
		int down = thick;
		int left = thick;
		int right = thick;
		...
		int padding = ((PADDING_BMP_SIZE - (new_width * sizeof(rgb)) % PADDING_BMP_SIZE) % PADDING_BMP_SIZE);
		if (new_width <= 0 || new_height <= 0){
			failure("New size of image less then 0");
		}
		unsigned int new_file_size = new_height*(new_width * sizeof(rgb) + padding)+ sizeof(image->bheader)+sizeof(image->dheader);
		...
		for (int i = 0; i < new_height; i++){
	  		new_image->data[i] = (rgb*)malloc(new_width * sizeof(rgb) + padding);
		}
		int x0 = 0;
		int y0 = 0;
