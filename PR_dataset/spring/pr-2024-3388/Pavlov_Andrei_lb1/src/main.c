...
int grab_pair(char *str, char **path, char **file)
{
...
void do_all_work(char **text, int len)
{
    char *path;
    char *file;
...
{
    int len = 0;
    char **text = text_read(&len);
    do_all_work(text, len);
    return 0;
}
