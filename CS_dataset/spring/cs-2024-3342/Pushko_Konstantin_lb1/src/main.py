...
class Edition:
    ...
    def __init__(self, name, price, age_limit, style):
        if isinstance(name,str) and self.isNatural(price) and self.isNatural(age_limit) and (style in ['c', 'b']):
            self.name = name
            self.price = price
...
class Book(Edition):
    def __init__(self, name, price, age_limit, style, author, hardcover, pages):
        ...
        if isinstance(author,str) and isinstance(hardcover,bool) and self.isNatural(pages):
            self.author = author
...
class Newspaper(Edition):
    def __init__(self, name, price, age_limit, style, online_edition, country, frequency):
        super().__init__(name, price, age_limit, style)
        if isinstance(online_edition,bool) and isinstance(country,str) and self.isNatural(frequency):
