...
class Book(Edition):  # Наследуется от класса Edition
    def __init__(self, name: str, price: int, age_limit: int,
                 style: str, author: str, hardcover: bool, pages: int):
...
class Newspaper(Edition):  # Наследуется от класса Edition
    def __init__(self, name: str, price: int, age_limit: int,
                 style: str, online_edition: bool, country: str, frequency: int, ):
        super().__init__(name, price, age_limit, style)
        if isinstance(country, str) and isinstance(online_edition, bool) and (
