class Edition:
    ...
    price - цена (в руб., целое положительное число)
    age_limit - возрастное ограничение (в годах, целое положительное число)
    style - стиль(значение может быть одной из строк: c (color), b(black))
...
class Book(Edition): #Наследуется от класса Edition
    ...
    age_limit - возрастное ограничение (в годах, целое положительное число)
    style - стиль(значение может быть одной из строк: c (color), b (black))
    author - автор (фамилия, в виде строки)
    hardcover - твердый переплет (значениями могут быть или True, или False)
...
class Newspaper(Edition): #Наследуется от класса Edition
    ...
    age_limit - возрастное ограничение (в годах, целое положительное число)
    style - стиль(значение может быть одной из строк: c (color), b (black))
    online_edition - интернет издание (значениями могут быть или True, или False)
    country - страна (строка)
    frequency - периодичность (период выпуска газеты в днях, целое положительное число)
