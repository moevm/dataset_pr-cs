class Node:
    ...
    def __str__(self):
        if self.next != None: return f'data: {self.data}, next: {self.next.get_data()}'
...
class LinkedList:
    ...
    def append(self, element):
        ...
        if len(self) == 0: self.__init__(new_element)
        else:
            head = self.head
            for i in range(len(self) - 1): head = head.next
    ...
    def pop(self):
        if len(self) == 0: raise IndexError('LinkedList is empty!')
            ...
            head = self.head
            for i in range(len(self) - 2): head = head.next
            head.next = None
    ...
    def change_on_start(self, n, new_data):
        if n > len(self) or n <= 0: raise KeyError(f"{n} doesn't exist!")
        head = self.head
