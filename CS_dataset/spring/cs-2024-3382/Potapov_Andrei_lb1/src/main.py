class Character():
    def __init__(self, gender, age, height, weight):
        if not ((gender == 'm' or gender == 'w') and isinstance(age, int) and age > 0 and isinstance(height,
                                                                                                     int) and height > 0 and isinstance(
                weight, int) and weight > 0):
...
class Warrior(Character):
    def __init__(self, gender, age, height, weight, forces, physical_damage, armor):
        ...
        if not (isinstance(forces, int) and forces > 0 and isinstance(physical_damage,
                                                                      int) and physical_damage > 0 and armor > 0):
            raise (ValueError("Invalid value"))
    ...
    def __eq__(self, other):
        return (self.physical_damage == other.physical_damage) and (self.forces == other.forces) and (
...
class Archer(Character):
    def __init__(self, gender, age, height, weight, forces, physical_damage, attack_range):
        super().__init__(gender, age, height, weight)
        if not (isinstance(forces, int) and forces > 0 and isinstance(physical_damage,
                                                                      int) and physical_damage > 0 and isinstance(
            attack_range, int) and attack_range > 0):
            raise (ValueError("Invalid value"))
