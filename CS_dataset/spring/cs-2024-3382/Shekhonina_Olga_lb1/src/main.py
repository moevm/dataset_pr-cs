class Edition:
    def __init__(self, name, price, age_limit, style):
        if type(name) == str and type(price) == int and price > 0 and type(age_limit) == int and age_limit > 0 and (style == 'c' or style == 'b'):
            self.name = name
            self.price = price
...
class Book(Edition):
    def __init__(self, name, price, age_limit, style, author, hardcover, pages):
        ...
        if type(author) == str and type(hardcover) == bool and type(pages) == int and pages > 0:
            self.author = author
...
class Newspaper(Edition):
    def __init__(self, name, price, age_limit, style, online_edition, country, frequency):
        super().__init__(name, price, age_limit, style)
        if type(online_edition) == bool and type(country) == str and type(frequency) == int and frequency > 0:
