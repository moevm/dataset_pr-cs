class Figure:
    def __init__(self, perimeter, area, color):
        if (isinstance(perimeter,int) and perimeter > 0 and isinstance(area, int) and area > 0 and (color =="r" or color == 'g' or color == 'b')):
...
class PolygonList(list):
    ...
    def print_colors(self):
        k = 1
        for p in self:
            print(f'{k} многоугольник: {p.color}')
            k+=1
...
class CircleList(list):
    ...
    def print_colors(self):
        for idx, item in enumerate(self, start=1):
            print(f'{idx} окружность: {item.color}')

    def total_area(self):
        total = sum(item.area for item in self)
        print(f'{total}')