...
class Warrior(Character):
    def __init__(self, gender, age, height, weight, forces, physical_damage, armor):
        super().__init__(gender, age, height, weight)  # Вызываем конструктор родительского класса
...
class MagicianList(list):
    ...
    def print_damage(self):
        ...
        for magician in self:
            total_damage += magician.magic_damage
        print(total_damage)
...
class ArcherList(list):
    ...
    def print_count(self):
        male_archers_count = sum(1 for archer in self if archer.gender == 'm')
        print(male_archers_count)
