class Edition:
    def __init__(self, name, price, age_limit, style):
        if not (isinstance(name, str) and isinstance(price, int) and price > 0 and isinstance(age_limit, int) and age_limit > 0 and (style == 'c' or style == 'b')):
...
class Newspaper(Edition):
    ...
    def __eq__(self, other):
        if self.country == other.country and self.name == other.name:
            return True
        else:
            return False
...
class BookList(list):
    ...
    def append(self, __p_object):
        if isinstance(__p_object, Book):
            super().append(__p_object)
        else:
            raise TypeError(f'Invalid type <тип объекта {type(__p_object)}>')
