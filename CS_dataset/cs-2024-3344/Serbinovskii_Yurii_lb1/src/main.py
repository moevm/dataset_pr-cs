...
class Book(Edition):
    ...
    def __str__(self):
        return (
            f'Book: название {self.name}, цена {self.price}, возрастное ограничение {self.age_limit}, стиль {self.style}, автор {self.author}, твердый переплет {self.hardcover}, количество страниц {self.pages}.')
...
class NewspaperList(list):
    ...
    def extend(self, iterable):
        super().extend([news for news in iterable if isinstance(news, Newspaper)])
