def check_requirements(variables):
    if (all(isinstance(param, int) for param in variables)):
        if (all(param > 0 for param in variables)):
            return True
    return False
...
class Character:
    def __init__(self, gender, age, height, weight):
        if ((gender == "m" or gender == "w") and check_requirements([age, height, weight])):
            self.gender = gender
            self.age = age
            self.height = height
            self.weight = weight
...
class Warrior(Character):
    def __init__(self, gender, age, height, weight, forces, physical_damage, armor):
        ...
        if (check_requirements([forces, physical_damage, armor])):
            self.forces = forces
...
class Magician(Character):
    def __init__(self, gender, age, height, weight, mana, magic_damage):
        super().__init__(gender, age, height, weight)
        if (check_requirements([mana, magic_damage])):
            self.mana = mana
...
class Archer(Character):
    def __init__(self, gender, age, height, weight, forces, physical_damage, attack_range):
        super().__init__(gender, age, height, weight)
        if (check_requirements([forces, physical_damage, attack_range])):
            self.forces = forces
            self.physical_damage = physical_damage
